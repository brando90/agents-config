...
**Authorized sign-in:** For an authorized account task, use the existing session or matching password-manager login, then retrieve a current verification code from available authorized email, Google Messages or authenticator tools when needed. Verify the account and official sign-in flow, keep credentials transient, and ask only for an actual human-only or unavailable step. Tool restrictions and final-action boundaries remain binding. Check access in the current runtime; publication alone does not update running sessions. Follow [authorized sign-in and verification codes](INDEX_RULES.md#authorized-sign-in-and-verification-codes).
