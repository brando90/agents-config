...
7. **Auto-update tools** — Claude Code and Codex auto-update on every session start via `SessionStart` hook in `~/.claude/settings.json` (which calls `~/agents-config/scripts/auto-update-tools.sh`). If a tool seems outdated or a feature is missing, run `bash ~/agents-config/scripts/auto-update-tools.sh` manually and restart.
