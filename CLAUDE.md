...
7. **Agent CLI freshness** — `SessionStart` in `~/.claude/settings.json` calls `~/agents-config/scripts/auto-update-tools.sh`. Read that as a general reminder to keep agent CLIs fresh, not as an exhaustive list of the only tools that exist: a machine may have `codex`, `claude` / `clauded`, `gemini`, or other vendor tools from Cursor, Mistral, Harmonic, Axiom, and future additions. If one looks stale or is missing a feature, use the matching package manager when possible; otherwise print a one-line natural-language reminder with the exact update command instead of guessing.
