...
**CV edits** (`~/brandomiranda/professional_documents/cvs/`): see **Trigger Rule 28** in `INDEX_RULES.md` — consistency invariants for `cv_short.tex` / `cv_long.tex` (descending chronology, bolded full-form venue lines, author order, `\flushbottom`, sync, recompile-and-verify, audience framing for the active grant).
