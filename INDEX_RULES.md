...
12. **Email experiment results to Brando immediately.** _Trigger: an experiment finishes (PASS or FAIL)._ Send an email to `brando.science@gmail.com` (CC `brando9@stanford.edu`) with the results. Do not ask, do not draft, do not skip — just send it. Use the template in [`workflows/expts-and-results.md`](workflows/expts-and-results.md) § Email Notification. This is how Brando tracks progress when away from the terminal. A finished experiment with no email is invisible work.
