...
17. **Default: run end-to-end without mid-task confirmations.** _Trigger: any user-assigned task (experiments especially)._ Run the entire task to completion before reporting back. Do not halt for verification after smoke tests, partial steps, or intermediate decisions. If a smoke test fails or any mid-task check is genuinely needed, self-verify using the user's original prompt + `~/CLAUDE.md` as ground truth and delegate to another model — Codex CLI (`codex exec --full-auto`), Gemini CLI, or a fresh Claude Code session (`clauded -p`) — instead of asking the user. Always use SOTA models (Guideline 19). Default flow: smoke → full E2E → QA (Hard Rule 3) → commit + `git push` (Trigger Rule 8) → only then surface to the user. Halt at the smoke stage only when the user explicitly says so ("smoke only", "stop after smoke", "verify before E2E"). When in doubt, keep going.
