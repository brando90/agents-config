...
3. **Run the two-step QA chain before reporting any non-trivial task as done.** Design: **AGENT1 → [AGENT1, AGENT2, AGENT3] → AGENT1** — whichever agent did the work self-reviews, dispatches the other CC/Codex agent and Gemini, then aggregates all verdicts. Roles swap based on who starts. One QA chain per user-assigned task, not per commit. Step 1: correctness ([`workflows/qa-correctness.md`](workflows/qa-correctness.md)) — uses **3-model parallel review** (CC self-review, Codex, and Gemini CLI), then the initial agent aggregates verdicts and makes the final call. Must pass before step 2. Step 2: structural ([`workflows/qa-structural.md`](workflows/qa-structural.md)) — skipped for markdown-only repos. **Dispatch all 3 reviewers in parallel.** If a reviewer is unavailable, continue with the remaining ones (minimum 1). The initial agent (the one that did the task) aggregates all verdicts: unanimous PASS → PASS; 2+ FAIL → FAIL; single dissent → initial agent investigates and decides. When unsure whether to run QA, run it. **Quick dispatch:** `codex exec --full-auto "$QA_PROMPT"`, `gemini -p "$QA_PROMPT"`, and CC self-review. See [`workflows/qa-correctness.md`](workflows/qa-correctness.md) for the full protocol.
...
- [`workflows/qa-correctness.md`](workflows/qa-correctness.md) — 3-model parallel correctness review: CC + Codex + Gemini CLI (step 1, default-on)
