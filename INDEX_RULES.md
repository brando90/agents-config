...
13. **Load ML writing guide when editing LaTeX.** _Trigger: editing `.tex` files for an ML research paper._ Read [`writing/ml_research_writing.md`](writing/ml_research_writing.md) into your context before making any edits. Follow its persona, abstract structure, and writing rules throughout the editing session.
14. **Email experiment results AND big-task completions to Brando immediately.** _Trigger: (a) an experiment finishes (PASS or FAIL), OR (b) a "big" user-assigned task finishes._ Send an email to `brando.science@gmail.com` (CC `brando9@stanford.edu`) with the results. Do not ask, do not draft, do not skip — just send it. Use the template in [`workflows/expts-and-results.md`](workflows/expts-and-results.md) § Email Notification (experiments) or § Big-Task Notification (non-experiment tasks). This is how Brando tracks progress when away from the terminal. Finished work with no email is invisible work.
...
15. **Email Q&A: answer questions in-chat AND email each answer.** _Trigger: the user asks a question in their prompt (substantive question — not rhetorical or agent-directed like "can you do X?")._ For every distinct question: (1) answer it in the normal response, (2) send a separate email per question. **Subject:** `Q → A: <brief question> → <brief answer>`. **Body:** Start with `TLDR: <1 sentence>`, then write in natural conversational prose — like emailing a colleague. Greet with "Hey Brando,", state the answer directly, add bullet points for concrete changes/details if relevant, sign off with `— Claude Code`. Keep it scannable and easy to read — no rigid `Question:` / `Answer:` labels. **From:** `brando.science@gmail.com` (SMTP login is `brandojazz@gmail.com` with `~/keys/gmail_app_password.txt`, but set `From:` header to `brando.science@gmail.com`). **To:** `brando.science@gmail.com` and `brando9@cs.stanford.edu`. Do NOT send to `brandojazz@gmail.com`. When a prompt contains multiple questions, send one email per question. Skip for trivial confirmations ("ok?", "right?") or meta-questions about the agent itself ("can you do this?").
