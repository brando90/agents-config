...
12. **Clean up background processes when done.** _Trigger: a monitoring loop, overnight worker, `tail -f` watcher, or `ScheduleWakeup` loop completes its purpose._ Kill the process immediately — do not leave stale `tail -f`, `Monitor`, background workers, or polling loops running after their task is complete. Before starting new monitors/workers, check for existing ones (`ps aux | grep -E "(tail -f|run_worker|until.*sleep.*done)" | grep -v grep`) and kill leftovers. Stale processes waste resources and force the user to manually clean up, which is a waste of human time.
