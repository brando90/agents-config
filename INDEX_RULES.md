...
3. **Run the two-step QA chain before reporting any non-trivial task as done.** One QA chain per user-assigned task, not per commit. Step 1: correctness ([`workflows/qa-correctness.md`](workflows/qa-correctness.md)) — **3-model parallel review: CC → [CC, Codex, Gemini] → CC** (or if Codex built: Codex → [Codex, CC, Gemini] → Codex). The builder self-reviews, dispatches the other two, then aggregates all three verdicts. Must pass before step 2. Step 2: structural ([`workflows/qa-structural.md`](workflows/qa-structural.md)) — dispatches 1 other agent, skipped for markdown-only repos. If a reviewer is unavailable, continue with the remaining ones (minimum 1). When unsure whether to run QA, run it. See [`workflows/qa-correctness.md`](workflows/qa-correctness.md) for the full protocol.
