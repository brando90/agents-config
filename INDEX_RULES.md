...
7. **Agent CLI freshness.** Claude Code's `SessionStart` hook (`~/.claude/settings.json`) runs `~/agents-config/scripts/auto-update-tools.sh` asynchronously on every session start. Treat this as a general freshness check, not a hardcoded list of the only agent tools that matter: the installed CLIs on a machine may include `codex`, `claude` / `clauded`, `gemini`, and other vendor tools from Cursor, Mistral, Harmonic, Axiom, or future additions. If a tool seems outdated (missing a feature, slash command, or expected behavior), detect how that binary was installed and either update it through the matching package manager or emit a one-line natural-language reminder naming the tool and the exact command the human should run. Never assume `npm` for a binary installed via Homebrew or another manager, and never let stale tooling block work.
