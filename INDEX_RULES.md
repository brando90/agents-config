...
17. **Default: run experiments end-to-end, do not halt at smoke tests.** _Trigger: a smoke test / dry-run / minimal-config pass succeeds._ Proceed automatically through full E2E → QA (Hard Rule 3) → commit + push (Trigger Rule 8). Do **not** stop and ask the user after the smoke test. Only halt at the smoke stage when the user explicitly says so ("smoke only", "stop after smoke", "verify before E2E", or similar). When in doubt, keep going — the user's default preference is full E2E without confirmation prompts.
