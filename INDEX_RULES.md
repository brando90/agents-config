...
7. **Auto-update agent tools.** Claude Code's `SessionStart` hook (`~/.claude/settings.json`) runs `~/agents-config/scripts/auto-update-tools.sh` asynchronously on every session start — this updates Claude Code and Codex CLI to latest. If you notice a tool is outdated (e.g., missing a feature or slash command), run `bash ~/agents-config/scripts/auto-update-tools.sh` manually and restart the session. Never let stale tooling block work.
