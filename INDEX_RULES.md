...
- [`workflows/dfs-job-watcher.md`](workflows/dfs-job-watcher.md) — DFS job queue daemon for SNAP nodes without Slurm. Smart mode (default) wraps jobs in a coding agent that diagnoses failures, retries, and emails results. Code: `~/ultimate-utils/py_src/uutils/job_scheduler_uu/`. **Prerequisite:** `~/dfs` must be a symlink to `/dfs/scratch0/<user>` on every node that runs or submits to the watcher — see SNAP Required Symlinks section above. **Checking jobs:** when asked "is my job running" / "check jobs," always inspect BOTH the watcher queue AND detached tmux/nohup/GPU processes (on source node and peers) — see `workflows/dfs-job-watcher.md` § "Is any job of mine running?"
