...
| Discord | ✅ connected on Mac + mercury2; Telegram-to-Discord not ready | Bot created (ID `1498169663278813254`, handle `@ultimate_brando9_bot`). Token lives only in `~/keys/openclaw_discord_bot_token.txt` / `~/keys/discord_bot_token.txt` (mode 600); OpenClaw config uses a file-backed SecretRef. Verified 2026-06-04 on local Mac and `mercury2.stanford.edu`; mercury2 requires `@openclaw/discord@2026.5.7` with OpenClaw 2026.5.7. Status may show `intents:content=limited`; if inbound ordinary message content fails, enable Message Content Intent in the Discord Developer Portal and confirm the bot is in the target server. Telegram-to-Discord testing status and commands live in [`runbook.md`](./runbook.md#telegram-bot-to-discord-test). |
...
1. Invite `@ultimate_brando9_bot` to a private Discord test server/channel or provide a concrete target ID, then run the [`runbook.md`](./runbook.md#telegram-bot-to-discord-test) raw Discord CLI send test before attempting Telegram-to-Discord.
