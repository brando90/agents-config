...
## Architecture v2 (2026-04-26): three instances + Telegram-as-substrate

**This section supersedes the 2-instance assumptions in §"Plan" above.** The original plan was 2 instances (mercury2 + Mac mini); we are now committing to **3 instances**: mercury2 + MacBook Air + MacBook Pro. The structural changes that follow are mandatory:

- **Channel: Telegram only.** WhatsApp Business multi-device cap is 4. With 3 OpenClaw instances + Brando's phone + (likely) his Mac WhatsApp = 5 devices. WhatsApp is dropped from this experiment. Telegram has no device limit and `openclaw channels add --channel telegram --token …` is non-interactive.
- **Heartbeat: private Telegram channel `openclaw-ops`.** Original spec used `~/dfs/openclaw_heartbeat/<host>.txt`, but neither Mac mounts DFS. Each instance posts `[host] alive @ <ts>` every 15 min into the channel; lifecycle events post `STARTING` / `RECOVERED`. A separate watcher posts `[<peer>] SILENT >30min` if a peer is silent past threshold. Reuses the chat substrate — no extra infra.
- **Idempotency stale-claim TTL: 5 min** (was 30 min). With 3 readers racing on one inbox, the original TTL leaves too wide a window for double-pickup. Tighten to 5 min and have each instance re-check labels immediately before commit, not just at pickup.
- **Auth replication: per-machine `codex login`.** Don't copy `~/.codex/auth.json` between hosts — refresh tokens rotate and copying snapshots breaks whichever host last refreshed. Each box runs `codex login` once.
- **Daemon by OS (no one-size-fits-all):**
  - Air + Pro: `openclaw onboard --install-daemon` writes `~/Library/LaunchAgents/ai.openclaw.gateway.plist`. Patch the plist with `NODE_EXTRA_CA_CERTS=/etc/ssl/cert.pem` + `NODE_OPTIONS=--dns-result-order=ipv4first --use-system-ca` (Homebrew Node + macOS DNS combination otherwise breaks outbound `fetch` for some hosts).
  - mercury2: SNAP doesn't enable systemd-user lingering. Use the existing tmux + watchdog while-loop + `@reboot` cron pattern from `~/agents-config/machine/snap.md`. Plus `krenew` cron for Kerberos token refresh per `init_no_passwords_snap_kinit.md`.
- **Config sync: via agents-config, not a separate repo.** Live in `experiments/01_self_hosted_openclaw/{config,scripts}/`. Each host pulls + re-runs `scripts/update_openclaw_instance.sh`. Secrets (Telegram bot token, Gmail token) stay in `~/keys/` (mode 600), `scp`'d between hosts, never committed. See [`setup-tutorial.md`](./setup-tutorial.md) for the full reproducible recipe and [`todos.md`](./todos.md) for the live deployment checklist.
...
|------|--------|-------|--------|-------|
| 2026-04-26 | claude-code (mac-air) | 0–1 | Air partially working | Codex-Pro-backed install live on the Air (gateway under launchd, `openclaw infer model run --gateway` returns `PONG` via `openai/gpt-5.5` on codex harness, Telegram bot `@ultimate_brando9_sk_air_bot` (renamed 2026-05-08 from `@ultimate_brando9_bot`) paired and chats with Brando). Open: gateway-side `openclaw message send --channel telegram` still fails with `HttpError: Network request for 'sendMessage' failed!` despite `NODE_EXTRA_CA_CERTS` + `NODE_OPTIONS=--dns-result-order=ipv4first --use-system-ca` + `NODE_TLS_REJECT_UNAUTHORIZED=0` all set in plist; agent runtime's reply path through grammy works fine, so this only blocks operator CLI not the feature itself — parked, see `todos.md`. Architecture v2 (this section) committed alongside. Reproducible recipe in `setup-tutorial.md`. Living TODO checklist in `todos.md`. Per-host install/update scripts and config templates in `scripts/` and `config/`. Drop-in triage agent prompt in `config/agent-prompt.md`. Pending Brando: Gmail OAuth (`openclaw channels add --channel google`), admin-filter list (edit `config/admin-filter.txt`), MacBook Pro + mercury2 SSH details for replication. |
