% TLDR: Optional LaTeX presentation of an experiment report. Experiment-folder Markdown (.md) reports are primary and sufficient; copy this only when a LaTeX report is wanted.
...
%  10. Experimental Details — model IDs, dates, local report/evidence paths; existing external links are optional
...
\begin{itemize}
  ...
  \item \textbf{Result files:}
    ...
    \end{itemize}
  % Optional existing W&B URL; create/update a report only on explicit user request.
  % \item \textbf{Optional external report:} \url{https://wandb.ai/brando-su/TODO}
\end{itemize}
