#!/usr/bin/env bash
# Install a node-local Stanford-enterprise Claude runtime using a separately authorized setup-token grant.
...
grant="$shared/.claude-su-remote/oauth-token"
[ -d "$node_home" ] || { echo 'Node-local home unavailable' >&2; exit 1; }
[ ! -L "$grant" ] && [ -s "$grant" ] || { echo 'Stanford remote grant missing' >&2; exit 1; }
[ "$(stat -c %u "$grant")" = "$(id -u)" ] && [ "$(stat -c %a "$grant")" = 600 ] || {
  echo 'Stanford grant must be owned by this user with mode 600' >&2; exit 1;
}
...
runtime="$node_home/.local/share/claude-su-runtime/$sha"
for dir in "$node_home/.local" "$node_home/.local/share" "$node_home/.local/bin" "$node_home/.local/share/claude-su-runtime" "$runtime" "$node_home/.claude-su-node"; do
...
chmod 700 "$node_home/.claude-su-node" "$node_home/.local/share/claude-su-runtime" "$runtime"
for cfg in settings.json .claude.json .credentials.json; do
  [ ! -L "$node_home/.claude-su-node/$cfg" ] || { echo 'Refusing symlink profile file' >&2; exit 1; }
done
...
profile="$node_home/.claude-su-node"
...
for command_name in claude-su clauded-su; do
  tmp=$(mktemp "$node_home/.local/bin/.su.XXXXXX")
  cat > "$tmp" <<WRAPPER
#!/usr/bin/env bash
set -euo pipefail
# Dedicated Stanford enterprise grant; never inherit personal, Vals or API billing credentials.
...
if [ -L '$grant' ] || [ ! -s '$grant' ] || [ "\$(stat -c %u '$grant')" != "\$(id -u)" ] || [ "\$(stat -c %a '$grant')" != 600 ]; then
 echo 'Protected Stanford remote grant unavailable' >&2; exit 1
fi
CLAUDE_CODE_OAUTH_TOKEN="\$(cat '$grant')" || { echo 'Cannot read Stanford grant' >&2; exit 1; }
[[ "\$CLAUDE_CODE_OAUTH_TOKEN" =~ ^sk-ant-oat[0-9]{2}-[A-Za-z0-9_-]{32,}$ ]] || { echo 'Invalid Stanford grant format' >&2; exit 1; }
export CLAUDE_CODE_OAUTH_TOKEN
if [ "\$(basename "\$0")" = clauded-su ]; then
...
printf 'SU_NODE_READY host=%s runtime=%s profile=%s\n' "$(hostname -s)" "$runtime/claude" "$profile"
