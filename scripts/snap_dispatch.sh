...
remote_logdir() { echo "/lfs/${1}/0/brando9/snap_jobs"; }

preflight() {
  local host="$1"
  ssh "${SSH_OPTS[@]}" "${host}.stanford.edu" true 2>/dev/null \
...
cmd_run() {
  ...
  [[ -n "$job_cmd" ]] || die "no command given"
  [[ "$name" =~ ^[A-Za-z0-9_.-]+$ ]] || die "job name must be alphanumeric/._- only"
  ...
  if ssh "${SSH_OPTS[@]}" "${SNAP_HOST}.stanford.edu" "tmux has-session -t '$name' 2>/dev/null"; then
  ...
  ssh "${SSH_OPTS[@]}" "${SNAP_HOST}.stanford.edu" bash -s -- \
      "$name" "$logdir" "$log" "$payload" <<'REMOTE'
set -euo pipefail
name="$1"; logdir="$2"; log="$3"; payload="$4"
mkdir -p "$logdir"
runner="$logdir/${name}_runner.sh"
{
...
REMOTE
  ...
  follow  : SNAP_HOST=$SNAP_HOST $0 tail $name
  attach  : SNAP_HOST=$SNAP_HOST $0 attach $name
  stop    : SNAP_HOST=$SNAP_HOST $0 kill $name
...
cmd_tail() {
  local name="${1:-}"; [[ -n "$name" ]] || die "usage: tail <job-name>"
  preflight "$SNAP_HOST"
  local logdir; logdir="$(remote_logdir "$SNAP_HOST")"
  ssh -t "${SSH_OPTS[@]}" "${SNAP_HOST}.stanford.edu" \
    "tail -f '${logdir}/${name}_latest.log'"
}

cmd_log() {
  local name="${1:-}"; [[ -n "$name" ]] || die "usage: log <job-name>"
  local logdir; logdir="$(remote_logdir "$SNAP_HOST")"
  ssh "${SSH_OPTS[@]}" "${SNAP_HOST}.stanford.edu" \
    "ls -la '${logdir}/${name}'*.log 2>/dev/null; echo '--- tail ---'; tail -40 '${logdir}/${name}_latest.log' 2>/dev/null || echo '(no log yet)'"
}

cmd_attach() {
  local name="${1:-}"; [[ -n "$name" ]] || die "usage: attach <job-name>"
  echo "Attaching to '$name' on $SNAP_HOST. Detach with Ctrl-b then d (do NOT Ctrl-c)."
  ssh -t "${SSH_OPTS[@]}" "${SNAP_HOST}.stanford.edu" "tmux attach -t '$name'"
}

cmd_kill() {
  local name="${1:-}"; [[ -n "$name" ]] || die "usage: kill <job-name>"
  ssh "${SSH_OPTS[@]}" "${SNAP_HOST}.stanford.edu" \
...
cmd_nodes() {
  ...
  for h in "${ALL_NODES[@]}"; do
    ssh "${SSH_OPTS[@]}" "${h}.stanford.edu" bash -s <<'REMOTE' 2>/dev/null || printf '%-11s (unreachable)\n' "$h"
