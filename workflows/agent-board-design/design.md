...
The implementation currently covers personal/Vals Claude profiles, a limited local Codex list and three hardcoded cluster names. The former six-hour history default and eight-Codex-task cap were implementation limits, not requirements. The follow-up removes the count cap and uses seven days of recent history while retaining registry/process-verified active sessions regardless of age. Full desktop/cloud lifecycle coverage remains a separate implementation phase. New cluster Vals profiles live at `/lfs/<node>/0/brando9/.claude-vals-node`; account recognition must be configurable. Desktop tasks sharing one application-server process need their native task identifiers; a process-count heuristic is insufficient.
