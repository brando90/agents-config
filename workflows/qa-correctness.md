...
**Model QA runs through verified subscription-authenticated clients, preferably `codex` and `claude` / `clauded`.** Trigger Rule 48 also authorizes supported Google-model clients, Cursor-hosted models, Grok or another provider for eligible roles after target-host, model, tools, data-access and billing verification. The deprecated Gemini CLI remains unsupported. This broadens available providers, not acceptance eligibility; explicitly required families/models remain required.

**Never fall back to application programming interface (API) keys or buying additional credits.** Do not set `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, or similar to make review work. Hard Rule 8 and [reliable dispatch](reliable-agent-dispatch.md#2-budget-for-finishing-including-review-and-publication) permit explicitly authorized existing balances through approved authenticated clients, with verified protection against new automatic/postpaid charges and the recorded task budget. This does not change reviewer eligibility, required families or attempt limits. An already-running conversation does not prove a fresh invocation can run. Recover an actual authentication failure through [the supported recovery procedure](reliable-agent-dispatch.md#authentication-recovery); ask for a human step only when it is actually required.
...
4. If the ordinary review still lacks an eligible result, dispatch a verified strongest eligible model from another working approved subscription in a **fresh context**, using the master's Rule 48 candidates. Prefer an eligible other-company substitute where available; it may be the builder's company when necessary. A third-provider candidate must have evidenced capability, exact model/effort and the tools for this review; an unverified alias or convenient availability is insufficient. For critical changes, go directly to this strongest-model substitute after an unrecoverable failure, unless the required company/model itself is mandatory. Do not automatically add an advisory smaller-model call: use one only when its limited critique is useful within the same attempt budget, and mark it advisory. Never count it as critical acceptance. Maximum **three invocations per required stage** (primary, optional one retry/alternative, one strongest-model substitute), not an unbounded model ladder.
...
verify the first reviewer's changes). Default is 3 stages, **Claude + Codex**. September 11 provider recovery permits only substitutes eligible under the canonical procedure, preserving explicitly required families and capability floors. The default has one independent reviewer, then the builder/self
